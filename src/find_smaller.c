...
/*   Updated: 2023/08/29 16:52:57 by wmillett         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	check_a(t_stacks *a, int index, int pos)
{
	...
	while (i < a->a_len)
	{
		if (a->a_stack[i] < a->b_stack[index]
			&& a->a_stack[i] > a->b_stack[pos])
		{
			check = i;
			break ;
		}
...
static int	get_small_a(t_stacks *a, int index, int pos)
{
	...
	while (i < a->b_len)
	{
		if (a->b_stack[i] < a->b_stack[index]
			&& a->b_stack[i] > a->a_stack[pos])
			return (INVALID);
		i++;
	}
	return (pos);
